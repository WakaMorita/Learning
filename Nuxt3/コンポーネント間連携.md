...
Componetsフォルダ内はインポート不要で利用可能
(サブフォルダ内を含む：サブフォルダ名＋コンポーネント名)
...
<details><summary>参考：サブフォルダ内のコンポーネント名</summary>

```rb
＊サブフォルダ内のコンポーネント名にはサブフォルダ名を含めることを推奨している
１．components/PartsTheSupplement.vue
２．components/parts/PartsTheSupplement.vue
３．components/parts/TheSupplement.vue
４．components/PartsTheSupplement.vue
読み込み順は　1→2→3　4は読み込まれない
```
</details>
